	...
	document.getElementById("editTaskForm").addEventListener("submit", async function(event) {
		...
		try {
			...
			if (data.success) {
				...
				row.querySelector(".task-description").innerText = formData.description;

				// Öncelik değerini Türkçeye çevirme
				let priorityText = {
					low: "Düşük",
					medium: "Orta",
					high: "Yüksek",
				...
				priorityBadge.innerHTML =
					`<span class="badge 
                    ${formData.priority === 'low' ? 'bg-success' : formData.priority === 'medium' ? 'bg-warning text-dark' : 'bg-danger'} 
                    fs-6 fw-normal">
                    ${priorityText[formData.priority] || formData.priority}
                </span>`;

				row.querySelector(".task-due_date").innerText = formData.due_date;
